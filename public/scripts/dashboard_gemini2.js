...
const CONFIG = {
  API_BASE_URL: "https://script.google.com/macros/s/AKfycbwjmnBDZcMdBmP6Dj67S19qGDP61ujNtBvJZU65xqlUfluThOy1pphwjvACS9FVXJeD/exec",
    ...
    // Lista completa e aggiornata di tutti gli ID DOM usati
    DOM_REFS: {
        ...
        bulletinBoardSection: 'bulletinBoardSection',
...
// --------------------------------------------------------------------
// 2. SERVIZIO API (APIService Class) - ADATTATO A GOOGLE APPS SCRIPT
// --------------------------------------------------------------------
class APIService {
    constructor(baseUrl) {
    ...
    // Corrisponde a doGet(e) sul Apps Script
    ...
    // Corrisponde a doPost(e) sul Apps Script
    async post(action, body = {}) {
        const res = await fetch(this.baseUrl, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // Invia l'action nel corpo JSON, come richiesto da doPost
...
const UIManager = {
    ...
    showGlobalLoader: () => { UIManager.refs.globalLoader?.classList.add('active'); },
    hideGlobalLoader: () => { UIManager.refs.globalLoader?.classList.remove('active'); },
    ...
    escapeBackticks: (str) => String(str).replace(/`/g, '\\`'), // Per stringhe template in HTML inline
    normalizeUrl: (u) => {
...
class DashboardApp {
    ...
    async handleManualLogin(e) {
        ...
        try {
            ...
            if (UIManager.refs.globalLoader?.classList.contains('active')) {
                UIManager.hideGlobalLoader();
            }
        }
    }
    ...
    async handleCallLogSubmission(e) {
        ...
        const data = Object.fromEntries(new FormData(UIManager.refs.callLogForm).entries());
        ...
        // Mappa i parametri per il backend (studentName -> studentId)
        const payload = {
            coachId: this.#coach.id,
            studentId: data.studentName, // ID dello studente
            date: data.date,
            callType: data.callType,
            // Aggiungi tutti gli altri campi del form se il tuo backend li gestisce
            ...data
        };
        // Rimuovi la chiave non necessaria dal payload
        delete payload.studentName;

        try {
            // Endpoing corretto: logCall
    ...
    async fetchAndRenderStudentDetails(studentId) {
        ...
        try {
            // Student ID è corretto qui, viene usato dal select element
    ...
    renderStudentDetailsHTML(studentId, info, calls, contracts) {
        const infoHtml = Object.entries(info).map(([key, value]) => {
            ...
            return `<div class="p-2 border-b"><span class="font-semibold">${displayKey}:</span> <span>${UIManager.escapeHTML(value)}</span></div>`;
        }).join('');
        ...
        return `
            ...
            <div class="bg-white shadow rounded-lg p-6 mb-8">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead><tr><th>Product</th><th>Start Date</th><th>End Date</th><th>Link</th></tr></thead>
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead><tr><th>Date</th><th>Type</th><th>Debrief</th></tr></thead>
    ...
    async handleSaveDraftDebrief(e) {
        ...
        const data = Object.fromEntries(new FormData(UIManager.refs.debriefForm).entries());
        ...
        // Mappa i parametri per il backend
        const payload = {
            coachId: this.#coach.id,
            studentId: data.studentName, // ID dello studente
            ...data
        };
        delete payload.studentName; // Rimuovi la chiave non necessaria

        UIManager.showGlobalLoader();
        try {
            // Endpoint corretto: saveDebrief (usato anche per i draft)
            const resp = await this.#api.post('saveDebrief', payload);
    ...
    async handleLoadDraftDebrief() {
        ...
        try {
            // GET request usa studentId nel parametro
            const resp = await this.#api.get('getDebriefDrafts', { coachId: this.#coach.id, studentId: student, date: date });
            ...
            return `
                <div class="border p-3 mb-2 rounded flex justify-between items-center bg-gray-50">
                    ...
                    <button class="bg-blue-500 hover:bg-blue-600 text-white text-sm py-1 px-3 rounded"
                        onclick="window.appInstance.applyDraftToForm(${UIManager.escapeHTML(JSON.stringify(draft))})">
                        Load
                    </button>
                </div>
            `;
    ...
    async handleSendDebrief(e) {
        ...
        const payload = {
            coachId: this.#coach.id,
            studentId: data.studentName, // ID dello studente
            isFinal: true, // Flag per indicare al backend che è la sottomissione finale
            ...data
        };
        delete payload.studentName;
    ...
    async handleAIPreview(fieldName) {
        ...
        try {
            // Endpoint corretto: generateDebriefText
            const resp = await this.#api.post('generateDebriefText', { 
                coachId: this.#coach.id, 
                studentId: student, // ID dello studente
                fieldName: fieldName, 
                context: content 
            });
                    ...
                    <div class="border p-4 rounded bg-yellow-50">
                        ...
                        <div class="mt-4 flex gap-2">
                            <button class="bg-green-500 hover:bg-green-600 text-white py-1 px-3 rounded"
                                onclick="window.appInstance.applyAISuggestion('${fieldName}', \`${UIManager.escapeBackticks(resp.suggestion)}\`)">Apply</button>
                            <button class="bg-red-500 hover:bg-red-600 text-white py-1 px-3 rounded"
                                onclick="window.appInstance.discardAISuggestion()">Discard</button>
                        </div>
                    </div>
                `;
            } else {
    ...
    buildSmartDeck(cards) {
        ...
        cards.forEach(card => {
            ...
            const score = Math.max(card.score || 1, 1);
    ...
    updateFlashcardScore(card, status) {
        ...
        if (status === 'good') {
            newScore = Math.min((newScore || 1) + 1, 15);
        } else if (status === 'bad') {
            newScore = Math.max((newScore || 1) - 1, 1);
        }
    ...
    async sendFlashcardUpdates() {
        ...
        try {
            const updates = this.#pendingFlashcardUpdates.map(u => ({
                en: u.en, score: u.score, lastReview: new Date().toISOString().split('T')[0],
            }));
            
            const studentId = UIManager.refs.flashcardsStudentSelect.value;
            // Endpoint corretto: updateFlashcardStatus
            const resp = await this.#api.post('updateFlashcardStatus', { studentId, updates });
    ...
    handleRetryReds() {
        ...
        const renderNextRed = () => {
            ...
            // Renderizza la carta successiva dopo l'aggiornamento
            UIManager.refs.flashcardFront.textContent = UIManager.escapeHTML(next.en);
            UIManager.refs.flashcardBack.textContent = UIManager.escapeHTML(next.it);
            UIManager.refs.flashcardCardContainer.classList.remove('flipped');
        };
    ...
    initEventListeners() {
        // Rendi accessibili le funzioni globali per l'HTML inline
        window.appInstance = this;
        window.handleGoogleLogin = this.handleGoogleLogin.bind(this);
        window.handleAIPreview = (fieldName) => this.handleAIPreview(fieldName);
        window.applyAISuggestion = (fieldName, suggestion) => this.applyAISuggestion(fieldName, suggestion);
        ...
        // Aggiunti i listener per le nuove sezioni
        document.getElementById('viewBulletinBoardBtn')?.addEventListener('click', () => UIManager.switchSection(CONFIG.DOM_REFS.bulletinBoardSection));
        document.getElementById('viewReportCardsBtn')?.addEventListener('click', () => UIManager.switchSection(CONFIG.DOM_REFS.reportCardsSection));
        document.getElementById('viewTeachingMaterialsBtn')?.addEventListener('click', () => UIManager.switchSection(CONFIG.DOM_REFS.teachingMaterialsSection));
